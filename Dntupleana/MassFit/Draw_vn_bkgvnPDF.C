...
void Draw_vn_ratio( TH1D * vn_default, TH1D * vn_bkgvnpoly2, TString vnname, TString method, TString MBorDtrig, int cent_low, int cent_high, float ptlow, float pthigh)
{
	...
	h_vn_diff_bkgvnpoly2_default->GetYaxis()->SetRangeUser(-0.1,0.1);
	...
	cfg_vn_effcorrection->SaveAs(Form("Plots_vn/plots_effcorrection/cfg_vn_bkgvnpoly2_%s_cent%dto%d.pdf", (vnname+"_"+method+"_"+MBorDtrig).Data(), cent_low, cent_high));
}
