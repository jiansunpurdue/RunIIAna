...
TF1* fit_histo_expobkg( bool isPbPb, int centlow, int centhigh, TH1D * histo, TH1D * h_mc_matched_signal, TH1D * h_mc_matched_kpiswapped, int ipt, TString cfgname)
{
	...
	TF1* f = new TF1(Form("f_%s_%d",cfgname.Data(),ipt),"[0]*([5]*([4]*Gaus(x,[1],[2])/(sqrt(2*3.14159)*[2])+(1-[4])*Gaus(x,[1],[3])/(sqrt(2*3.14159)*[3]))+(1-[5])*Gaus(x,[1],[6])/(sqrt(2*3.14159)*[6])) + [7] * exp([8]*x)", fit_range_low, fit_range_high);
	...
	TF1* background = new TF1(Form("background%d",ipt),"[0] * exp([1]*x)");
	...
	TF1* mass = new TF1(Form("fmass%d",ipt),"[0]*([5]*([4]*Gaus(x,[1],[2])/(sqrt(2*3.14159)*[2])+(1-[4])*Gaus(x,[1],[3])/(sqrt(2*3.14159)*[3])))");
	...
	TF1* massSwap = new TF1(Form("fmassSwap%d",ipt),"[0]*(1-[2])*Gaus(x,[1],[3])/(sqrt(2*3.14159)*[3])");
	...
	if(isPbPb)
	{
		cfg->SaveAs(Form("Massfitplots/PbPb/DMass_isPbPb%d_%s_%d_expobkg.pdf", isPbPb, cfgname.Data(), ipt));
		//cfg->SaveAs(Form("Massfitplots/PbPb/DMass_isPbPb%d_%s_%d_expobkg.png", isPbPb, cfgname.Data(), ipt));
	}
	else
	{
		cfg->SaveAs(Form("Massfitplots/pp/DMass_isPbPb%d_%s_%d_expobkg.pdf", isPbPb, cfgname.Data(), ipt));
		//cfg->SaveAs(Form("Massfitplots/pp/DMass_isPbPb%d_%s_%d_expobkg.png", isPbPb, cfgname.Data(), ipt));
	}
